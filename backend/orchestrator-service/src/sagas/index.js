...
(async () => {
  ...
  await consumer.subscribe({ topic: 'service3-response', fromBeginning: true });
  /* 
  ...
  consumer.run({
    eachMessage: async ({ topic, message }) => {
      ...
      if (topic === 'service1-response') {
        ...
        await producer.send({
          topic: 'service3',
          messages: [{ value: JSON.stringify({ action: 'process-ev3', payload: response }) }],
        });
      } else if (topic === 'service3-response') {
        console.log('END SAGE with received from service3:', response);
      }
    },
  });
})();
